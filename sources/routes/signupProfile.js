...
const modelSignupProfile = require('../models/modelSignupProfile');
...
router.get('/', async function(req, res) {
        ...
        try {
                const result = await ctrlRead.displayAllSignup(modelSignupProfile.modelSignupProfile);
                if (result == '[]') {
...
router.get('/signupProfileId', async function(req, res) {

        const result = await ctrlRead.findUserByID(modelSignupProfile.modelSignupProfile, req.body._id)
        return res.status(200).send(result);
...
router.post('/create',
        ...
        async function(req, res) {
                try {
                        const resultModelSignup = new modelSignupProfile.modelSignupProfile(req.body);
...
router.delete('/delete', async function(req, res) {
        try {
                ...
                const result = await ctrlDelete.findUserAndDelete(modelSignupProfile.modelSignupProfile, req.body._id);
...
router.put('/update', async function(req, res) {
        try {
                const result = await ctrlUpdate.updateValueById(modelSignupProfile.modelSignupProfile, req.body._id, req.body);
