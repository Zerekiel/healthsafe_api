...
const ctrlEmailTools = require('../controllers/ctrlEmailTools');
...
const birthDayValidator = (jsonValue) => {
	const regexp = /^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/i;
	return eValidator.check(jsonValue)
		.exists().withMessage("Error -> Doesn't exist")
		.not().isEmpty().withMessage("Error -> is empty")
		.matches(regexp).withMessage("Error -> Doesn't matches(regex)")
		.custom((value) => ctrlTools.birthDay(value))
			.withMessage("Error -> Age under 18 or invalid birthday.");
}

const birthDayValidatorPUT = (jsonValue) => {
	const regexp = /^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/i;
	return eValidator.check(jsonValue)
		.matches(regexp).withMessage("Error -> Doesn't matches(regex)")
		.custom((value) => ctrlTools.birthDay(value))
